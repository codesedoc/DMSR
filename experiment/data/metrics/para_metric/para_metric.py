...
class ModelBasedMetric(datasets.Metric):
    ...
    def _compute(self, predictions, references, batch_size=100, silence=False):
        """Returns the scores"""
        set_diversities = []
        seq_diversities = []
        diversity_scores = []
        total=len(predictions)
        for pred, ref in tqdm(zip(predictions, references), total=total, desc="edit_distance", disable=False):
