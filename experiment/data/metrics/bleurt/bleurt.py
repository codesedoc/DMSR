...
class BLEURT(datasets.Metric):
    ...
    scores = []
    def _compute(self, predictions, references, batch_size=64, device=None, adjusted=True, silence=False):
        """Returns the scores"""
        if device != self.device and device is not None:
            self.set_device(device)

        scores = []
        for preds, refs in tqdm(zip(to_batches(predictions, batch_size), to_batches(references, batch_size)), total=int(ceil(min(len(predictions),len(references)) / batch_size)), desc="bleurt", disable=silence):
